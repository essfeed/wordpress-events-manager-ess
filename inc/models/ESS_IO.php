...
final class ESS_IO
{
	...
	const ROBBY_WEBSITE	    = 'http://www.robby.ai';
	...
	public static function set_activation()
	{
        ...
        if (!wp_next_scheduled ( ESS_IO::CRON_EVENT_HOOK )) {
		    wp_schedule_event( time(), 'daily', ESS_IO::CRON_EVENT_HOOK ); //daily | hourly | tenminutely
        }
	}
	...
	public static function set_deactivation()
	{
		...
		wp_clear_scheduled_hook( ESS_IO::CRON_EVENT_HOOK );
	}
	...
	public static function set_admin_submenu_page()
	{
		$plugin_pages = array();
	   	...
	   	$plugin_pages['help'] = add_submenu_page(
	   		...
	   		__('Import/Export events with ESS','em-ess'),
	   		__('ESS Feed','em-ess'),
	   		'list_users',
	...
	public static function get_curl_result( $target_url, $feed_url='' )
	{
		...
		$post_data = array(
			...
			'PROTOCOL'      => ( ( EM_ESS_SECURE )? 'https://' : 'http://' ),
			'fast'			=> ( ( defined( 'EM_ROBBY_VERSION' ) == TRUE )? 'true' : 'false' ),
			...
			else
			{
				...
				if ( $result == FALSE )
				{
					...
					$ESS_Notices->add_error(
						__( "PHP cURL must be installed on your server or PHP parameter 'allow_url_fopen' must be set to TRUE: ", 'em-ess' ).
