...
final class ESS_Images
{
	...
	public static function add( Array $DATA_=NULL )
	{
		...
		if ( $h && strlen( $url ) > 5 )
		{
			...
			if ( file_exists( $file[ 'tmp_name' ] ) && ESS_Images::image_validator( $file ) )
			{
				...
				if ( $attachment )
				{
					...
					return apply_filters( 'em_object_image_upload', $attachment_id, ( isset( $this ) )? $this : 'self' );
				}
			}
			@fclose( $h );
			if ( file_exists( $tmpfname ) )
		...
		return apply_filters( 'em_object_image_upload', false, ( isset( $this ) )? $this : 'self');
	}
	...
	private static function image_validator( $file=NULL )
	{
		...
		if ( !empty( $file ) && $file[ 'size' ] > 0 )
		{
			...
			if (   $file['size']  > $maximum_size ) 											{ array_push( $ERRORS_, sprintf( __( 'The image file is too large! Maximum size: %s, You can change thit limitation in Events > Settings > General > Images Sizes', 'em-ess' ), $maximum_size ) ); }
		  	if ( ( $file['width'] > $maximum_width) || ( $file['height'] > $maximum_height ) ) 	{ array_push( $ERRORS_, sprintf( __( 'The image is too large! Maximum size allowed: %s, You can change thit limitation in Events > Settings > General > Images Sizes','em-ess' ), $maximum_width ." x ". $maximum_height ) ); }
			if ( ( $file['width'] < $minimum_width) || ( $file['height'] < $minimum_height ) ) 	{ array_push( $ERRORS_, sprintf( __( 'The image is too small! Minimum size allowed: %s, You can change thit limitation in Events > Settings > General > Images Sizes','em-ess' ), $minimum_width ." x ". $minimum_height ) ); }
		  	if ( empty( $file['type'] ) || !array_key_exists( $file['type'], $mime_types ) ) 	{ array_push( $ERRORS_, __( 'The image is in a wrong format! The format allowed are GIF, PNG and JPG', 'em-ess' ) ); }
  		}
