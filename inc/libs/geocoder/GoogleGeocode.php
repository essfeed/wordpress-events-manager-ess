...
class GoogleGeocode extends BaseGeocode
{
	...
	public static function getGeocodeFromAddress( $address )
	{
		$url = "http://maps.google.com/maps/api/geocode/json?".
				"address=".urlencode( $address )."&sensor=false";
		
		//echo __METHOD__." > url: " . $url . "<br/>";
	...
	public static function getGeocodeFromLatLng( $lat, $lng )
	{
		$LOCATION_ = array();
		...
		if ( strlen( $lat ) > 0 && strlen( $lng ) > 0 )
		{
			...
			//$url = "http://api.geonames.org/findNearbyPlaceName?lat=".$lat."&lng=" .$lng."&username=demo";
			//echo "url: ". $url;
            ...
            {
            	if ( @count( $result[ 'results' ][ 0 ][ 'address_components'] ) > 0 )
				{
					foreach( $result[ 'results' ][ 0 ][ 'address_components'] as $i => $geoObj )
					{
						if ( @count( $geoObj['types'] ) > 0 )
						{
							foreach ( $geoObj['types'] as $geotype ) 
							{
								switch ( strtolower( $geotype ) )
								{
									case 'administrative_area_level_1' : 
										$LOCATION_[ 'state' ] 			= $geoObj['short_name'];
										$LOCATION_[ 'state_code' ] 		= $geoObj['long_name'];
										break;
									case 'country'	:
										$LOCATION_[ 'country_code' ] 	= $geoObj['short_name'];
										$LOCATION_[ 'country' ]			= $geoObj['long_name'];
										break;
									case 'postal_code' :
										$LOCATION_[ 'zip' ] 			= $geoObj['short_name'];
										break;
									case 'locality' :
										$LOCATION_[ 'city' ] 			= $geoObj['long_name'];
										break;
								}
							}
						}
				...
				if ( @$result[ 'results' ][ 0 ]['geometry']['location']['lat'] )
				{
					$LOCATION_[ 'latitude' ] 	= @$result[ 'results' ][0]['geometry']['location']['lat'];
                	$LOCATION_[ 'longitude' ] 	= @$result[ 'results' ][0]['geometry']['location']['lng'];
				}
