...
final class ESS_Import
{
		...
		if ( ESS_Import::is_feed_valid( $feed_url ) )
		{
			...
			if ( strlen( @$RESULT_['error'] ) <= 0 || @$RESULT_['error'] == NULL )
			{
				...
				if ( @count( @$RESULT_['feeds'] ) > 0 && $RESULT_['feeds'] != NULL )
				{
					...
					//		__("The URL of the feed defined within the feed is not the same then the one you have aggregate.",'em-ess') .
					//		"<br/>".
					...
					*/
					...
					if ( @count( $real_event_ids_ ) > 0 )
					{
						...
						) ) === TRUE )
						{
							...
							$ESS_Notices->add_confirm( sprintf( __( "The ESS feed have been %s.", 'em-ess' ), __( ( intval( $feed_id ) > 0 )? "updated" : "created", 'em-ess' ) ) );
						}
						else $ESS_Notices->add_error( __( "Impossible to insert the ESS Feed in the Data Base: ", 'em-ess' ) . ESS_Elements::get_ahref( $feed_url ) );
					}
					else $ESS_Notices->add_error( __( "No events found in the ESS Feed:", 'em-ess' ). ESS_Elements::get_ahref( $feed_url ) );
				}
				else $ESS_Notices->add_error( __( "Impossible to analyse the ESS Feed: ",'em-ess' ). ESS_Elements::get_ahref( $feed_url ) );
			}
				...
				if ( $EM_Event->can_manage( 'edit_events', 'edit_recurring_events', 'edit_others_events' ) && $EM_Event->get_post() ) // user must have permissions.
				{
					...
					if ( strlen( $_POST[ 'content' ] ) > 0 )
					{
						if ( get_option( 'ess_backlink_enabled' ) )
						{
							...
							$_POST['content'] .= "<h6>". __( "Source:", 'em-ess') . " <a title=\"". __( "Source:", 'em-ess') . " ".$feed_uri_host."\" href=\"" . $feed_uri . "\">" .  parse_url ( $feed_uri, PHP_URL_HOST ) . "</a></h6>";
						}
					}
					...
					if ( $_POST[ 'no_location' ] === FALSE && strlen( $_POST['location_name'] ) > 0 && get_option( 'dbem_locations_enabled' ) )
					{
						...
						if ( $EM_Location->can_manage( 'publish_locations' ) && $EM_Location->get_post(FALSE) )
						{
							...
							if ( property_exists( $EM_Location, 'owner' ) == FALSE )
								$EM_Location->owner = $EM_Location->location_id;
					...
					if ( @count( $_POST['em_tickets'] ) > 0 && get_option('dbem_rsvp_enabled') )
					{
						...
						if ( @count( $EM_Tickets->tickets ) <= 0 )
						{
							foreach( $_POST['em_tickets'] as $ticket_data )
							{
								...
								if ( property_exists( $EM_Ticket, 'owner' ) == FALSE )
									$EM_Ticket->owner = $EM_Ticket->ticket_id;
					...
					if ( @count( $_POST['event_categories'] ) > 0 && get_option( 'dbem_categories_enabled' ) )
					{
						...
						if ( $EM_Categories->can_manage( 'edit_event_categories' ) )
						{
							...
							if ( property_exists( $EM_Categories, 'owner'   ) == FALSE &&
							     property_exists( $EM_Categories, 'term_id' ) == TRUE
                            )
							{
								$EM_Categories->owner = $EM_Categories->term_id;
                            }
						}
						else
						{
					...
					if ( @count( $_POST[ 'event_tags' ] ) > 0 && get_option( 'dbem_tags_enabled' ) )
					{
						...
						if ( property_exists( $EM_Tags, 'owner'   ) == FALSE &&
                             property_exists( $EM_Tags, 'term_id' ) == TRUE
                        )
						{
							$EM_Tags->owner = $EM_Tags->term_id;
                        }
					} // end add tags
					//dd( $EM_Tags );
					...
					if ( strlen( $_POST[ 'event_start_date' ] ) > 0 )
					{
						...
						if ( $EM_Event->end < date( 'U' ) )
							$ESS_Notices->add_info( sprintf( __( "An event imported is already finished: %s", 'em-ess' ), ESS_Elements::get_ahref( @$FEED_[ 'generals' ]['uri'] ) ) );
					...
					// -- force owner ID setting
					if ( property_exists( $EM_Event, 'owner' ) == FALSE )
						$EM_Event->owner = $EM_Event->event_id;
									...
									if ( $EM_Event->can_manage( 'upload_event_images' ) )
									{
										...
										if ( FeedValidator::getMediaType( $media_[ 'uri' ] ) == 'image' && get_option('ess_feed_import_images' ) )
										{
											...
											if ( $attachment_id !== FALSE && intval( $attachment_id ) > 0 )
											{
												...
												$ESS_Notices->add_error( __( "Impossible to upload the event's image: ", 'em-ess' ).ESS_Elements::get_ahref( $media_[ 'uri' ] ) );
										} // end add images
										...
										if ( FeedValidator::getMediaType( $media_[ 'uri' ] ) == 'video' && get_option('ess_feed_import_videos' ) )
										{
											if ( ESS_Videos::add( array(
												...
												$ESS_Notices->add_error( __( "Impossible to upload the event's video: ", 'em-ess' ).ESS_Elements::get_ahref( $media_[ 'uri' ] ) );
										} // end add videos
										...
										if ( FeedValidator::getMediaType( $media_[ 'uri' ] ) == 'sound' && get_option('ess_feed_import_sounds' ) )
										{
											if ( ESS_Sounds::add( array(
												...
												$ESS_Notices->add_error( __( "Impossible to upload the event's audio file: ", 'em-ess' ).ESS_Elements::get_ahref( $media_[ 'uri' ] ) );
										} // end add sounds
									}
									else $ESS_Notices->add_error( $EM_Event->get_errors() );
								}
								else $ESS_Notices->add_info( sprintf( __( "A media file defined in the ESS feed is not reachable: <a href='%s' target='_blank'>%s</a>", 'em-ess' ), $media_[ 'uri' ], $media_[ 'uri' ] ) );
							}
	...
	public static function is_feed_valid( $feed_url='' )
	{
		...
		{
			$ESS_Notices->add_error( sprintf( __( "The ESS URL is not valid: <a href='%s' target='_blank'>%s</a>", 'em-ess' ), $feed_url, $feed_url ) );
			return FALSE;
		}
		else
		{
			...
			if ( $response !== FALSE )
			{
				...
				if ( @isset( $r[ 'result' ] ) )
				{
					if ( strlen( @$r['result']['error'] ) > 1 || @$r['type'] == 'KO' )
					{
						...
						array_push( 	$response['result']['error'], "<b>" . sprintf( __( "More information about the standard: <a href='%s' target='_blank'>%s</a>", 'em-ess' ), ESS_IO::ESS_WEBSITE, ESS_IO::ESS_WEBSITE ). "</b><br>" );
