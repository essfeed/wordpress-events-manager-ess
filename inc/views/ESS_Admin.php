...
final class ESS_Admin
{
	...
	public static function main_page()
	{
        ...
        /*wp_enqueue_style('ess-admin',       EM_ESS_PATH . 'assets/css/admin.css',                FALSE, EM_ESS_VERSION, TRUE);*/

        if (EM_ESS_DEBUG == FALSE)
        {
            wp_enqueue_script('ess-settings',   EM_DIR_URI  . 'includes/js/admin-settings.js',       FALSE, EM_ESS_VERSION, TRUE );
            wp_enqueue_script('ess-toggles',    EM_ESS_PATH . 'assets/js/jquery.toggles.min.js',     FALSE, EM_ESS_VERSION, TRUE );
            wp_enqueue_script('ess-maphilight', EM_ESS_PATH . 'assets/js/jquery.maphilight.js',      FALSE, EM_ESS_VERSION, TRUE );
            wp_enqueue_script('ess-timezone',   EM_ESS_PATH . 'assets/js/jquery.timezone-picker.js', FALSE, EM_ESS_VERSION, TRUE );
            wp_enqueue_script('ess-admin',      EM_ESS_PATH . 'assets/js/admin.js',                  FALSE, EM_ESS_VERSION, TRUE );
        }

		?><style  type="text/css" charset="utf-8"><?php include( EM_ESS_DIR.'/assets/css/admin.css'); ?></style><?php

        if (EM_ESS_DEBUG == TRUE)
        {
            ?>
		...
		<div id="ess_loader" style="display:none;">
			...
			<div><?php _e( 'please wait...', 'em-ess' ); ?></div>
		</div>
		<div class="wrap">
			...
			<h2 class="nav-tab-wrapper">
				<a href="#export" id="em-menu-export" class="nav-tab nav-tab-active"><?php _e('Settings','em-ess'); ?></a>
				<a href="#import" id="em-menu-import" class="nav-tab"><?php _e('Syndication','em-ess'); ?></a>
				<a id="btAddESS" class="add-new-h2" title="<?php _e('Import an ESS Feed','em-ess'); ?>" style="display:none;">+</a>
				<a id="btViewErrors" class="add-new-h2" title="<?php _e('View Previous Errors','em-ess'); ?>" style="display:none;"><?php _e('errors','em-ess'); ?></a>
			</h2>
			<form id="em-options-form" method="post"  enctype='multipart/form-data' target="_self" onsubmit="ess_admin.loader();"><?php
				echo $ESS_Notices;
	...
	private static function get_feed_form()
	{
		...
		?><div id="add_feed_form" class="highlight" style="display:<?php echo ( ( ESS_Control_admin::is_form_import_ess_visible() )?'block;':'none;');?>">
			...
			<div id="titlediv">
				...
				<div class="iphone <?php echo ( ( isset( $_REQUEST['ess_feed_mode'] ) )? ( ( $_REQUEST['ess_feed_mode'] == 'on' )? 'on' : 'off' ) : 'off' );?>" id="ess_mode" data-checkbox="ess_feed_mode_checkbox" data-ontext="<?php _e('Update Daily','em-ess'); ?>" data-offtext="<?php _e('Import Once','em-ess'); ?>"><?php _e('Update Daily','em-ess'); ?></div>
				...
				<input type="submit" value="<?php _e('ADD','em-ess'); ?>" class="button-primary" id="bt_add_feed" />
			</div>
		</div><?php
	}
	...
	private static function get_nav_action()
	{
		...
		?><div class="tablenav top">
			<div class="alignleft actions">
				<select name="action">
					<option value="-1" selected="selected"><?php _e('Bulk Actions','em-ess'); ?></option>
					<option value="active"><?php echo (( empty( $view ) || $view == 'all' )?__('Move to Active','em-ess'):__('Restore','em-ess') );?></option>
					<option <?php echo (( $view == 'trash' )?'value="full_deleted">Delete Permanently':'value="deleted">'.__('Move to Trash','em-ess') );?></option>
					<?php if ( empty( $view ) || $view == 'all' ) : ?>
					<option value="update_cron"><?php _e('Save Daily Updates','em-ess'); ?></option>
					<?php endif; ?>
				</select>
				<input class="button action" type="submit" value="<?php _e('Apply','em-ess'); ?>" name="apply_ess_table_filter" />
			</div>
		</div><?php
	}
	...
	private static function import_page()
	{
		...
		?><div class="em-menu-import em-menu-group" style="display:none;">
			...
			<!-- PAGES NAV -->
			<div class="subsubsub">
				<a href='<?php echo $url_all_events; ?>' <?php echo ( empty( $view ) || $view == 'all' )? 'class="current"':''; ?>><?php _e ( 'All', 'em-ess' ); ?> <span class="count">(<?php echo $active_count; ?>)</span></a>
				<?php if ($trash_count>0):?>&nbsp;|&nbsp;
				<a href='<?php echo $url_trash_events; ?>' <?php echo ( $view == 'trash' )? 'class="current"':''; ?>><?php _e ( 'Trash', 'em-ess' ); ?> <span class="count">(<?php echo $trash_count;   ?>)</span></a>
				<?php endif ?>
			</div><?php
            ...
            foreach( get_option( 'cron' ) as $timestamp => $date_ )
			{
				...
				{
					$next_cron = $timestamp;
					//d( date( 'Y-m-d H:i', $timestamp ), $date_, ESS_Timezone::get_date_GMT( date('U') ), ESS_Timezone::get_date_GMT( $timestamp ), sprintf( __("in %s",'em-ess'), human_time_diff( $next_cron ) ) );
                    break;
				}
			}
			...
			<table class='widefat'>
				<thead>
					<tr>
						...
						<th width="18"></th>
						<th><?php             _e('Title', 		'em-ess') ?></th>
						<th><?php             _e('Nb Events', 	'em-ess') ?></th>
						<th><?php             _e('Owner', 		'em-ess') ?></th>
						<th width="90"><?php  _e('Update Daily','em-ess') ?></th>
						<th width="100"><?php _e('Last Update', 'em-ess') ?></th>
						<th width="100"><?php _e('Next Update', 'em-ess') ?></th>
						<th width="45"><?php  _e('Update', 		'em-ess') ?></th>
						<th width="50"><?php  _e('View',		'em-ess') ?></th>
					</tr>
				</thead>
				<tfoot>
					<tr>
						...
						<th><?php _e('Nb Events', 	'em-ess') ?></th>
						<th><?php _e('Owner', 		'em-ess') ?></th>
						<th><?php _e('Update Daily','em-ess') ?></th>
						<th><?php _e('Last Update', 'em-ess') ?></th>
						<th><?php _e('Next Update', 'em-ess') ?></th>
						<th><?php _e('Update',      'em-ess') ?></th>
						<th><?php _e('View', 		'em-ess') ?></th>
					</tr>
				</tfoot>
				<tbody><?php
					...
					if ( @count( $efi ) > 0 )
					{
						...
						?><tr class="<?php echo $class; ?>">
							...
							<td>
								<h4><?php echo sprintf( __("%s ago",'em-ess'), human_time_diff( strtotime( $feed->feed_timestamp ) ) ); ?></h4>
							</td>
							<td>
								<h4><?php echo ( ( $feed->feed_mode == ESS_Database::FEED_MODE_CRON && $next_cron != NULL )? sprintf( __("in %s",'em-ess'), human_time_diff( $next_cron ) ) : '-' ); ?></h4>
							</td>
							<td>
								<button title="<?php _e( "Reimport the feed to update the event.", 'em-ess' );?>" onmousedown="ess_admin.set_event_id('<?php echo $feed->feed_id; ?>');" class="button-primary reload_box" name="update_once" value="<?php echo urlencode( $feed->feed_url );?>" style="background-image:url('<?php echo EM_ESS_URL;?>/assets/img/reload_icon_24x24.png');background-position:7px 2px;background-repeat:no-repeat;"></button>
							</td>
							<td>
								<a href="<?php echo $feed->feed_url; ?>" target="_blank" title="<?php _e( "Download the ESS Feed.",'em-ess'); ?>">
					...
					{
						?><tr>
							<th colspan="8" style="text-align:left;"><?php echo _e( "No Feeds Found",'em-ess'); ?></th>
						</tr><?php
					}
				?></tbody>
			</table>
		...
		?><div class="em-menu-export em-menu-group">
			...
			<div id="poststuff">
				...
				<div class="postbox" id="em-ess-export-syndication">
					<div class="handlediv" title="<?php _e( 'Click to toggle', 'em-ess'); ?>"><br /></div><h3><span><?php _e( 'Syndication Settings', 'em-ess' ); ?> </span></h3>
					<div class="inside">
						...
						<p class="submit">
							<input type="submit" class="button-primary" name="save_export" value="<?php _e( 'Save Changes (All)', 'em-ess' )?>" />
						</p>
					</div>
				</div>
				...
				<div class="postbox" id="em-ess-export-feed">
					<div class="handlediv" title="<?php _e( 'Click to toggle', 'em-ess'); ?>"><br /></div><h3><span><?php _e( 'Feed Settings', 'em-ess' ); ?> </span></h3>
					<div class="inside">
						...
						<table class="form-table">
							<tbody>
								...
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e('Max. Events per Feed', 'em-ess'); ?></th>
									<td>
										...
										<em><?php _e('Some ESS aggregators limits the acceptable file size to 2Mo.', 'em-ess'); ?></em>
									</td>
								</tr>
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e( "Events' Purpose", 'em-ess' )?>&nbsp;</th>
									<td>
										...
										<em><?php _e('Defines the global theme of all your events (used by search engines)', 'em-ess'); ?></em>
									</td>
								</tr>
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e( "Events' Currency", 'em-ess' )?>&nbsp;</th>
									<td>
										...
										<em><?php _e("Defines the currency used to set the events' prices", 'em-ess'); ?></em>
									</td>
								</tr>
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e ( 'Feed Language:', 'em-ess' )?>&nbsp;</th>
									<td>
										<select name="ess_feed_language" style="max-width:402px;">
											<option value="0" <?php echo ( get_option('ess_feed_language') == '' ) ? 'selected="selected"':''; ?>><?php _e('none selected','em-ess'); ?></option>
								...
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e('Events Time Zone', 'em-ess'); ?></th>
				...
				<div class="postbox" id="em-ess-export-visibility">
					<div class="handlediv" title="<?php _e( 'Click to toggle', 'em-ess'); ?>"><br /></div><h3><span><?php _e( 'Feed Visibility', 'em-ess' ); ?> </span></h3>
					<div class="inside">
						...
						<table class="form-table">
							<tbody>
								...
								<tr><th colspan="3"><strong><?php _e("Website Feed Visibility","em-ess");?> </strong></th></tr>
								<?php self::get_checkbox_table_row( array(
								...
								<tr><th colspan="3"><strong><?php _e("Feed's Elements Visibility","em-ess");?> </strong></th></tr>
				...
				<div class="postbox" id="em-ess-export-owner">
					<div class="handlediv" title="<?php _e( 'Click to toggle', 'em-ess'); ?>"><br /></div><h3><span><?php _e ( 'Events Organizer', 'em-ess' ); ?> </span></h3>
					<div class="inside">
						...
						<table class="form-table">
							<tbody>
								<tr>
									<th><strong><?php _e('Display Organizer', 'em-ess'); ?></strong></th>
									<td>
										<?php ESS_Elements::button_checkbox( array( 'id' => 'ess_owner_activate', 'checked' => get_option('ess_owner_activate'), 'on'=>'ON', 'off'=>'OFF' ) );?>
										<em><?php _e('Display, in the feed, the coordinate of the event organizer (will be the same for all the events).', 'em-ess'); ?></em>
									</td>
								</tr>
							</tbody>
						</table>
						<table class="form-table" id="block_owner" style="opacity:<?php echo get_option('ess_owner_activate')?'1':'0.3';?>;">
							<tbody>
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e ( 'First Name / Last Name:', 'em-ess' )?>&nbsp;</th>
									<td>
								...
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e ( 'City / State / Postcode:', 'em-ess' )?>&nbsp;</th>
									<td>
								...
								<tr>
									<td width="20">&nbsp;</td>
									<th><?php _e ( 'Country:', 'em-ess' )?>&nbsp;</th>
									<td>
										<select name="ess_owner_country">
											<option value="0" <?php echo ( get_option('ess_owner_country') == '' ) ? 'selected="selected"':''; ?>><?php _e('none selected','em-ess'); ?></option>
								...
								<?php self::get_input_table_row( array( 'id' => 'ess_owner_website', 'title' => 'Website:', 'placeholder' => 'http://' ));?>
								<?php self::get_input_table_row( array( 'id' => 'ess_owner_phone',   'title' => 'Phone:',   'placeholder' => '+001' ));?>
							</tbody>
						</table>
						<p class="submit">
							<input type="submit" class="button-primary" name="save_export" value="<?php _e( 'Save Changes (All)', 'em-ess' )?>" />
				...
				<div class="postbox" id="em-ess-export-social">
					<div class="handlediv" title="<?php _e( 'Click to toggle', 'em-ess'); ?>"><br /></div><h3><span><?php _e ( 'Social Platform', 'em-ess' ); ?> </span></h3>
					<div class="inside">
						...
						<?php foreach ( ESS_Database::$SOCIAL_PLATFORMS as $type => $socials_ ) : ?>
							<div class="postbox" id="em-ess-export-social">
								<div class="handlediv" title="<?php _e( 'Click to toggle', 'em-ess'); ?>"><br /></div><h3><span><?php _e ( ucfirst( $type ), 'em-ess' ); ?> </span></h3>
								<div class="inside">
									<table class="form-table">
										<tbody>
	...
	private static function get_social_table_row( $social='' )
	{
		?><tr>
			<td width="20">&nbsp;</td>
			<th><?php _e( ucfirst( $social ), 'em-ess' ); ?></th>
	...
	private static function get_input_table_row( Array $DATA_=NULL )
	{
		?><tr>
			<td width="20">&nbsp;</td>
			<th><?php _e( $DATA_['title'], 'em-ess'); ?></th>
			<td><input type="text" name="<?php echo $DATA_['id']; ?>" value="<?php echo ESS_Database::get_option( $DATA_['id'] ); ?>" placeholder="<?php echo (isset($DATA_['placeholder']) ? $DATA_['placeholder'] : ""); ?>" /></td>
		</tr><?php
	}
	...
	private static function get_timezone_picker()
	{
		...
		<fieldset id="timezone-fieldset">
			...
			<select id="edit-date-default-timezone" name="ess_feed_timezone">
				<option value=""><?php _e('none selected','em-ess'); ?></option>
				<?php foreach ( ESS_Timezone::$WORLD_TIMEZONES_ as $TZ_ ) : ?>
