...
methods.init = function(initOpts) {
  ...
  return $origCall.each(function(index, item) {
    ...
    // Add the pin.
    if (opts.pinUrl) {
      $pin = $('<img src="' + opts.pinUrl + '" />').appendTo(imgElement.parentNode).css({display:'none',width:7,height:7});
    }
    else if (opts.pin) {
      $pin = $(imgElement).parent().parent().find(opts.pin).appendTo(imgElement.parentNode).css({display:'none',width:7,height:7});
    }
    ...
    $(mapElement).find('area').click(function() {
      ...
      if (opts.target) {
        ...
        if (timezoneName) {
        	...
        	$('#current-timezone').html(timezoneName.replace('/',' / ').replace('_',' '));
        }
      }
...
jQuery.fn.timezonePicker.defaults = {
